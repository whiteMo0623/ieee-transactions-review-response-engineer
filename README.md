...
- **结论优先写作**：内部完整分析疑虑，最终回复直接呈现结论、决定性证据和论文修改。
...
flowchart TD
    ...
    G --> I["按结论—证据—修改顺序撰写 Author Response"]
    H --> I
    I --> J["创建唯一相邻备份"]
    J --> K["原位写入指定 Word"]
    K --> L["使用 Microsoft Word 只读导出 QA PDF"]
    L --> N["回读目标块并渲染目标页面"]
    N --> M["清理临时产物并交付"]
```
...
内部方案可以充分分析疑虑、真实意图和证据门槛，但最终回复不复述处理过程。`Author Response` 按“直接结论 → 决定性证据 → 论文实际修改 → 必要结论边界”组织；第一句必须独立回答当前意见，不以致谢、认同、担忧复述或 `To address this concern` 一类套话开场。公式使用 `$...$` 表达，表格和图片只保留当前论证需要的信息。

写入 Word 前检查：删除开场句是否不损失事实；若不损失，直接删除。限制条件只在会改变结论范围时保留，并放在证据之后。
...
- Windows 环境优先使用本机 Microsoft Word 的只读 COM 导出生成临时 QA PDF；
- 使用独立隐藏的 Word 实例，不加入最近文件、不保存文档，也不影响用户已有的 Word 实例；
- 只渲染目标回复实际占用的页面；出现分页或布局异常时再扩大到相邻页；
- 不默认使用 LibreOffice；Word 不可用时须经用户明确同意，并说明 LibreOffice 的排版结果仅供近似检查；
